  ...
  if (error) {
    return (
      <div className="rounded-lg bg-red-50 border border-red-200 p-4">
        <p className="text-sm text-red-700">Error loading booking_pages: {error.message}</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <Link href="/dashboard/booking-pages/new" className="btn-primary">
      ...
      {items && items.length > 0 ? (
            ...
            <tbody className="divide-y divide-gray-200">
              {items.map((item: Record<string, unknown>) => (
                <tr key={item.id as string} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-900 whitespace-nowrap">{String(item.theme_color ?? "—")}</td>
                  <td className="px-6 py-4 text-sm text-gray-900 whitespace-nowrap">{String(item.is_active ?? "—")}</td>
                  <td className="px-6 py-4 text-right text-sm whitespace-nowrap">
                    <Link href={`/dashboard/booking-pages/${item.id}/edit`} className="text-brand-600 hover:text-brand-800 font-medium mr-4">
                      Edit
                    </Link>
                    <DeleteButton id={item.id as string} table="booking_pages" />
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="card text-center py-12">
          ...
          <h3 className="mt-4 text-sm font-semibold text-gray-900">No booking pages</h3>
          <p className="mt-1 text-sm text-gray-500">Get started by adding your first booking page.</p>
          <Link href="/dashboard/booking-pages/new" className="btn-primary mt-6 inline-flex">
            Add Booking Page
          </Link>
        </div>
      )}
