  ...
  if (!record) {
    return (
      <div className="rounded-lg bg-red-50 border border-red-200 p-4">
        <p className="text-sm text-red-700">Event Type not found.</p>
      </div>
    );
  }
    ...
    <div className="max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="card space-y-6">
        ...
        <div>
          ...
          <input id="duration_minutes" name="duration_minutes" type="number" className="input" defaultValue={String(record.duration_minutes ?? "")} required />
        </div>
        <div>
        ...
        <div>
          ...
          <input id="buffer_before_minutes" name="buffer_before_minutes" type="number" className="input" defaultValue={String(record.buffer_before_minutes ?? "")} />
        </div>
        <div>
          <label htmlFor="buffer_after_minutes" className="label">Buffer After Minutes</label>
          <input id="buffer_after_minutes" name="buffer_after_minutes" type="number" className="input" defaultValue={String(record.buffer_after_minutes ?? "")} />
        </div>
        <div>
          <label htmlFor="max_bookings_per_day" className="label">Max Bookings Per Day</label>
          <input id="max_bookings_per_day" name="max_bookings_per_day" type="number" className="input" defaultValue={String(record.max_bookings_per_day ?? "")} />
        </div>
        ...
        <div className="flex items-center gap-3 pt-4 border-t">
          <button type="submit" disabled={loading} className="btn-primary">
            {loading ? "Saving..." : "Update Event Type"}
          </button>
